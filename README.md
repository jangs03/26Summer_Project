...
### Chroma DB를 팀원과 공유하기 (공유 서버 모드)

팀 전체가 같은 벡터 DB를 실시간으로 보게 하려면, Chroma를 **서버 모드**로 띄우고 팀원들은 `HttpClient`로 접속합니다. `config.py`의 `CHROMA_MODE`가 이미 `"http"`로 기본 설정되어 있습니다.

**1) 서버 띄우기 (한 명 또는 상시 켜져 있는 공유 머신에서 1회)**
...
이 프로세스가 켜져 있는 동안 팀원들이 접속해서 조회/적재할 수 있습니다.

**2) 팀원들은 `config.py`에서 서버 주소만 지정**

```python
CHROMA_MODE = "http"
CHROMA_HOST = "<서버를 띄운 머신의 IP 또는 호스트명>"
CHROMA_PORT = 8000
```

**3) 네트워크 연결 방법 (택 1)**

- **Tailscale(추천, 무료)**: 팀원 전원이 Tailscale 설치 후 같은 네트워크에 join하면 사설 IP로 바로 접속 가능. 포트포워딩/공인 IP 불필요, 셋업이 제일 간단함.
- **같은 Wi-Fi/사내망**: 서버를 띄운 머신의 로컬 IP(`ifconfig`/`ipconfig`로 확인)를 팀원들에게 공유. 회의실/카페 등 같은 네트워크에 있을 때만 유효.
- **클라우드 VM**: 무료 티어(Oracle Cloud, Fly.io 등)에 올리면 특정인의 노트북 전원 상태와 무관하게 상시 접속 가능. 설정은 더 필요하지만 가장 안정적.

**주의**: `chroma run`은 기본적으로 인증이 없습니다. 팀 내부망/VPN으로만 열어두고, 공인 인터넷에 그대로 노출하지 않는 것을 권장합니다.

**운영 팁**: 여러 명이 동시에 `main.py --daily`를 각자 돌리면 같은 논문을 중복으로 재계산할 수 있습니다(결과 자체는 `upsert`라 깨지진 않지만 비효율적). 실제 수집·임베딩 파이프라인(`--backfill`, `--daily`)은 한 명(또는 서버 머신의 cron)만 실행하고, 나머지 팀원은 `embedder.search_similar()`로 조회만 하는 방식을 권장합니다.
